...
```{r cars}
spotify <- read.csv(file="/Users/miasung/Library/CloudStorage/OneDrive-UniversityofToronto/2025-2026/2025 Fall - Meth Data Analysis - STA302H1/spotify-2023.csv", header=T)
...
plot(spotify$bpm, spotify$danceability_., 
     ...
     col = "blue", pch = 19)
prelim <- lm(as.numeric(streams) ~ scale(energy_.) + scale(danceability_.) + scale(acousticness_.) + scale(speechiness_.) + scale(bpm) + scale(released_year) + scale(valence_.) + scale(liveness_.), data = spotify)
summary(prelim)
fitted_prelim <- fitted(prelim)
residuals_prelim <- resid(prelim)
par(mfrow = c(2, 2))
